                    ...
                    <v-col cols="12">
                        <v-card>
                            <v-alert outlined color="#365164">
                                ...
                                <v-row>
                                    <v-col cols="6">
                                        <v-text-field label="팀장 (학번)" v-model="teamLeader" @keypress="checkNumber" @keyup="checkHan" counter maxlength="8" hint="학번을 8자리 숫자로 입력하세요." />
                                    </v-col>
                                    <v-col cols="6">
                                ...
                                <v-row>
                                    <v-col cols="6">
                                        <v-menu ref="menu1" v-model="menu1" :close-on-content-click="false" :return-value.sync="startDate" transition="scale-transition" offset-y min-width="290px">
                                    ...
                                    <v-col cols="6">
                                        <v-menu ref="menu2" v-model="menu2" :close-on-content-click="false" :return-value.sync="endDate" transition="scale-transition" offset-y min-width="290px">
                                            <template v-slot:activator="{ on }">
                                                <v-text-field v-model="endDate" label="프로젝트 마감일" readonly v-on="on"></v-text-field>
                                            </template>
                                            <v-date-picker v-model="endDate" no-title scrollable>
                                                <v-spacer></v-spacer>
                                                <v-btn id="menu2" color="primary" @click="()=> {temp(); $refs.menu2.save(endDate); }">확인</v-btn>
                                            </v-date-picker>
                                        </v-menu>
                                    </v-col>
                                </v-row>
                                ...
                                <v-row>
                                    <v-col cols="12">
                                        <v-textarea v-model="contents" outlined no-resize height="300" name="content-input" label="프로젝트 내용을 입력하세요" />
                                    </v-col>
                                </v-row>

                                <v-row>
                                    <v-col>
                                        <v-file-input v-model="files" :roles="[rules.fileCheck]" accept="image/*" color="deep-purple accent-4" counter label="사진 추가" multiple placeholder="사진을 추가하려면 여기를 클릭하세요" prepend-icon="mdi-camera" outlined :show-size="1000" />
                                    </v-col>
                                </v-row>
