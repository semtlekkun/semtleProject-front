<template>
  <v-card class="mx-auto" id="menuBar">
    <v-app-bar dark>
      ...
      <v-toolbar-title>
        <v-img width="90px" class="mx-auto" src="../assets/menuBarLogo.png" />
      </v-toolbar-title>
    </v-app-bar>

    <v-navigation-drawer v-model="drawer" absolute :stateless="!drawer">
      <v-list-item>
        <v-list-item-content>
          <v-img
            class="mx-auto my-4"
      ...
      <v-list nav>
        <v-list-item-group active-class="text--accent-4">
          <router-link v-for="item in mainItems" :key="item.Title" :to="item.url">
            <v-list-item link class="py-4" @click="item.method">
          ...
          <div v-show="loginStatus">
            <router-link v-for="item in mainItemsLogIn" :key="item.Title" :to="item.url">
              <v-list-item link class="py-4" @click="item.method">
                <v-list-item-title class="text-center text-h5">{{item.Title}}</v-list-item-title>
              </v-list-item>
            </router-link>
          </div>
