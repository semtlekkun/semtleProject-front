<template>
  <v-container>
    <v-form @submit="CheckForm" novalidate="true">
      <!-- 작성자 정보 표시 (임시) -->
      <v-row class="mt-5">
        <v-col cols="12" md="3" offset-lg="2" lg="2">작성자 : {{writerId}}</v-col>
      </v-row>
      ...
      <v-row class="mt-5 justify-center">
        ...
        <v-col cols="12" md="9" lg="6">
          <v-text-field id="title_" label="제목" outlined v-model="title"></v-text-field>
        </v-col>
      </v-row>
      ...
      <v-row>
        <v-col cols="12" offset-md="3" md="9" offset-lg="4" lg="6">
          <v-card height="400" align="center" color="grey lighten-3">
            ...
            <div v-else>
              <br />
              {{imageUrl}}
              <v-img :src="imageUrl" width="400" height="300"></v-img>
      ...
      <v-row>
        <v-col xs="12" md="8" offset-xs="0" offset-md="2">
          <!-- 스낵바 -->
          <v-btn block x-large dark type="submit" color="primary" @click="check">
...
export default {
  ...
  methods: {
    CheckForm(e) {
      this.axios
        .post(
          "http://49.50.166.64/api/notice/input",
          {
            date: this.date,
            title: this.title,
