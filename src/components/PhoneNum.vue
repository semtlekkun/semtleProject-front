...
<script>
    export default {
        ...
        methods: {
            ...
            submit() {
                this.errMsg = [];
                if (this.phoneNum1.length < 3 || this.phoneNum2.length < 4 || this.phoneNum3.length < 4) {
                    this
                        .errMsg
                        .push("휴대폰 번호를 정확하게 입력해주세요.");
                }
                ...
                if (this.errMsg.length !== 0) {
                    ...
                    this
                        .axios
                        .put("http://49.50.166.64/api/mypage/phoneNum/update", sendObj, config)
                        .then((res) => {
                            if (res.status === 200) {
                                this.dialog = false;
                                this
